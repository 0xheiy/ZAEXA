...
(function () {
  ...
  /* ⚠️ هر سلکتور با `63` — یعنی PUSH4 — می‌آید، چون جدولِ توزیعِ سالیدیتی
     دقیقاً همین را تولید می‌کند. نسخه‌ی اول سلکتورها را با «dead» به هم
     می‌چسباند و هیچ PUSH4ی نداشت؛ یعنی ماک شکلی از بایت‌کد را بازی می‌کرد
     که در واقعیت وجود ندارد. تا وقتی اسکنر با زیررشته می‌گشت این تفاوت
     دیده نمی‌شد — و همان اسکنرِ زیررشته‌ای بود که روی سایت زنده به AERO
     ریسک ۷۷ داد. ماک باید واقعیت را آینه کند، وگرنه سبزیِ تست چیزی را
     اثبات نمی‌کند.
     طولِ همه‌ی تکه‌ها زوج است تا موقعیتِ هر سلکتور روی مرزِ بایت بماند. */
  const FAKE_CODE = "0x60806040" +
    DANGER_IN_CODE.map(s => "63" + fakeId(s).slice(2, 10)).join("dead") +
    "00".repeat(40);
